...
def _resource_path(name):
    ...
    重要：冻结态下本模块 __file__ 路径里的中文（AgentDesktop）可能被编码损坏（如变 СAI），
...
def _ensure_cdp(cdp, app_dir=None):
    ...
    if app_dir:
        ...
        profile = os.path.expandvars(r"%LOCALAPPDATA%/AgentDesktop/cdp_edge_profile")
    try:
    ...
    return False, ("自动启动 Edge 后调试端口仍未就绪。请稍后重试，或检查 Edge 是否被安全"
                   "软件拦截启动。Agent使用独立 profile 接管浏览器，不会干扰你的真实 Edge。")
