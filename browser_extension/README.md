# 小臭抓网页 · 浏览器扩展（Edge / Chrome）

把看网页时的一键抓取能力接进「小臭玩AI」桌面端，抓下来的正文/选中文字会进小臭输入框，让 AI 帮你总结、提取、分析。
...
4. 看到「小臭抓网页」即成功；可点「固定」把图标钉到工具栏。

## 配对（只需一次）

5. 打开小臭玩AI → 设置 → 浏览器扩展，复制里面的「配对码」。
6. 点工具栏的小臭图标，把配对码粘贴到「配对码」框里（自动保存）。

## 使用

- 在任意网页点工具栏图标 → 「抓取当前页正文」：把正文发到小臭。
- 先在网页里选中一段文字 → 点「抓取选中文字」：只发选中部分。
- 可选填「附言」作为指令，如「提取里面的表格」「用一句话总结」。
- 抓取后内容出现在小臭输入框，按 Enter 即可让 AI 处理。
...
## 进阶：让小臭直接操作你的真实浏览器（CDP 接管）

扩展只负责「读取/抓取」。如果你想让小臭**点击、填表、操作**你正在用的浏览器（带登录态），用 CDP：
...
3. 在小臭「设置 → 浏览器扩展」里把这段地址填进 `config.json` 的 `browser_cdp` 字段
   （值为 `http://127.0.0.1:9222` 即可，小臭会自动取 ws 地址）。
4. 之后对话里说「打开 xx 网页并点登录」「去 xx 填表」等，小臭就接管你真实浏览器操作。
