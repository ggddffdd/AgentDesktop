# -*- coding: utf-8 -*-
"""chat_web.py — v4.104：QWebEngineView 聊天渲染引擎。

替换 QTextBrowser 的三大动机：
1. QTextDocument 富文本引擎不支持 border-radius/max-width%/inline-block → 气泡直角、
   宽度失控（"不像成熟产品"的直接原因）；
2. 流式输出靠 setHtml 全量重建整段对话 → 越聊越卡 + 滚动条跳动；
3. 无 Markdown 渲染（代码块/列表/加粗全是纯文本）。

本模块：Chromium 真渲染（CSS 圆角/阴影/动画全支持），流式只更新最后一个气泡的
innerHTML（其余消息零重排），Markdown 走内嵌 marked.js 离线渲染。

JS 调用契约（Python → JS，均带 js 前缀）：
- jsAppend(html)        追加一条消息（不触碰已有 DOM）
- jsStream(html)        更新流式气泡（#stream-bubble，无则创建）
- jsEndStream()         移除流式气泡
- jsRenderAll(html)     全量重建（切换会话/重生成）
- jsClear()             清空
- jsForceBottom()       强制滚底 + 恢复 JS 跟随
- jsHighlight(q)        搜索高亮（q 空 = 清除）
- jsScrollTo(idx)       滚动到 #msg_{idx}（对话内搜索跳转）

Python 侧链接拦截（JS → Python）：
- <a href="app://regen/5"> 等 app:// 链接 → anchorActivated("regen:5")
- http(s) 链接 → 系统浏览器打开（QDesktopServices）
"""
import json
import os
...
# v4.120：WebEngine 运行诊断日志——白屏复发排查用。
# 记录：启动版本/GPU flags、渲染进程崩溃(status/exit_code)、探活失败、自愈重建。
...
# marked v12.0.2（MIT）——离线内嵌，35KB，无网络依赖
...
class _ChatPage(QWebEnginePage):
    """拦截 app:// 业务链接与 http 外链。
    app:// 主通道：JS click 监听 → console.log('__xc__kind:idx') → javaScriptConsoleMessage。
    备通道：acceptNavigationRequest（直接导航场景）。"""

    anchorActivated = Signal(str)

    def javaScriptConsoleMessage(self, level, message, line, source):
        if message.startswith("__xc__"):
            self.anchorActivated.emit(message[len("__xc__"):])
            return
    ...
    def acceptNavigationRequest(self, url, nav_type, isMainFrame):
        s = url.toString()
        if s.startswith("app://"):
            # app://regen/5 → "regen:5"（与旧 anchorClicked 契约一致）
            rest = s[len("app://"):]
            kind, _, idx = rest.partition("/")
            self.anchorActivated.emit(f"{kind}:{idx}" if idx else kind)
            return False
        if s.startswith(("http://", "https://")) and nav_type == QWebEnginePage.NavigationTypeLinkClicked:
            QDesktopServices.openUrl(url)
            return False
        return super().acceptNavigationRequest(url, nav_type, isMainFrame)


def _build_html(theme: dict) -> str:
    t = {k: v for k, v in theme.items() if isinstance(v, str)}
...
#stream-bubble.done .bubble::after {{ content: none; }}
/* 滚动条美化 */
::-webkit-scrollbar {{ width: 9px; }}
::-webkit-scrollbar-thumb {{ background: #c8ccd4; border-radius: 5px; border: 2px solid
  {t.get("bg", "#F7F8FC")}; }}
::-webkit-scrollbar-thumb:hover {{ background: #aab0ba; }}
::-webkit-scrollbar-track {{ background: transparent; }}
</style>
<script>{_MARKED_JS}</script>
</head>
<body>
<div id="container"></div>
<script>
var container = document.getElementById('container');
var follow = true;   // 粘性跟随：用户上滑即关，回到底部即开
if (window.marked) {{
  marked.setOptions({{ gfm: true, breaks: true }});
}}
function sanitize(el) {{
  // 防御：剥离脚本类标签与 on* 事件、javascript: 协议（marked 不转义内嵌 HTML）
  el.querySelectorAll('script,style,iframe,object,embed,link,meta').forEach(function(n){{ n.remove(); }});
...
function renderMd(root) {{
  ...
  root.querySelectorAll('[data-md]').forEach(function(n) {{
    try {{
      n.innerHTML = marked.parse(n.getAttribute('data-md'));
      sanitize(n);
    }} catch (e) {{ n.textContent = n.getAttribute('data-md'); }}
...
window.addEventListener('scroll', function() {{
  follow = (document.body.scrollHeight - window.scrollY - window.innerHeight) < 48;
}});
window.jsAppend = function(html) {{
  container.insertAdjacentHTML('beforeend', html);
  renderMd(container.lastElementChild || container);
  stick();
}};
window.jsStream = function(html) {{
  var b = document.getElementById('stream-bubble');
  if (!b) {{
    container.insertAdjacentHTML('beforeend',
      '<div class="msg-row ai" id="stream-bubble">' + html + '</div>');
  }} else {{
    b.innerHTML = html;
  }}
  renderMd(document.getElementById('stream-bubble'));
  stick();
}};
window.jsEndStream = function() {{
  var b = document.getElementById('stream-bubble');
  if (b) b.remove();
}};
window.jsRenderAll = function(html) {{
  container.innerHTML = html;
  renderMd(container);
  stick();
}};
window.jsClear = function() {{ container.innerHTML = ''; }};
window.jsForceBottom = function() {{
  follow = true;
  window.scrollTo(0, document.body.scrollHeight);
}};
window.jsHighlight = function(q) {{
...
window.jsScrollTo = function(idx) {{
  var el = document.getElementById('msg_' + idx);
  if (el) el.scrollIntoView({{ block: 'center', behavior: 'smooth' }});
}};
window.jsReplaceLive = function(id, html) {{
  var el = document.getElementById(id);
  if (el) {{ el.outerHTML = html; renderMd(document.getElementById(id) || container); stick(); }}
}};
// app:// 业务链接 → console 通道回传 Python（未知 scheme 的合成导航可能被
// Chromium 内部吞掉、不触发 acceptNavigationRequest，console 通道永远可靠）
document.addEventListener('click', function(e) {{
  ...
  if (href.indexOf('app://') === 0) {{
    e.preventDefault();
    console.log('__xc__' + href.substring(6).replace('/', ':'));
  }}
}});
</script>
</body></html>"""
...
class ChatWebView(QWebEngineView):
    """聊天渲染视图。所有 JS 调用在页面就绪前排入队列，loadFinished 后冲刷。
    页面意外重载（offscreen 环境偶发 loadFinished(False)→reload）会清空 DOM，
    届时发 pageReloaded 让上层全量重渲染自愈。"""
    ...
    def _on_render_process_terminated(self, status, exit_code):
        ...
        _we_diag(f"renderProcessTerminated status={status} exit_code={exit_code} -> rebuild")
        # v4.120 熔断：60 秒内崩 3 次 = 崩溃-重建死循环（如 GL 上下文全禁导致的
        # 启动即 Killed），继续重建只会每秒闪屏+吃满 CPU。停手等心跳探活兜底
        # （60s 一次、代价低），滑动窗口自然恢复。
        import time as _t
        now = _t.monotonic()
        self._crash_times = [t for t in getattr(self, "_crash_times", [])
                             if now - t < 60.0]
    ...
    def _breaker_retry(self):
        """熔断后的冷静重试：只在页面仍未就绪时重建一次。"""
        if self._ready:
            return
        _we_diag("breaker cool-down retry -> rebuild")
        self._ready = False
        self._rebuilding = True
        self.setHtml(_build_html(self._theme), QUrl("file:///"))
    ...
    def _on_probe_result(self, result):
        ...
        if result is True:
            self._probe_fails = 0
            return
        # v4.120 容错：None（页面忙/流式渲染中/回调异常）不等于页面死了——
        # 明确 False（DOM 真没了）记 2 分立即重建，None 记 1 分、连续 2 次才重建。
        # 旧逻辑「非 True 即重建」会把大 DOM 忙碌期的单次误报变成每分钟白屏闪一次。
        self._probe_fails += 2 if result is False else 1
        if self._probe_fails < 2:
            _we_diag(f"probe suspicious result={result!r} fails={self._probe_fails} -> tolerated")
            return
        self._probe_fails = 0
        _we_diag(f"probe dead result={result!r} -> rebuild")
        self._ready = False
        self._rebuilding = True
        self.setHtml(_build_html(self._theme), QUrl("file:///"))
    ...
    def _js_str(s: str) -> str:
        """Python str → JS 字符串字面量（JSON 序列化足够）。"""
        return json.dumps(s or "", ensure_ascii=False)

    # ---- 公共 API ----
    def append(self, html: str):
        self._exec(f"jsAppend({self._js_str(html)})")

    def update_stream(self, html: str):
        self._exec(f"jsStream({self._js_str(html)})")

    def end_stream(self):
        self._exec("jsEndStream()")

    def render_all(self, html: str):
        self._exec(f"jsRenderAll({self._js_str(html)})")

    def clear(self):
        self._exec("jsClear()")

    def force_bottom(self):
        self._exec("jsForceBottom()")

    def highlight(self, q: str):
        self._exec(f"jsHighlight({self._js_str(q)})")

    def scroll_to(self, idx):
        self._exec(f"jsScrollTo({int(idx)})")

    def replace_live(self, id_: str, html: str):
        self._exec(f"jsReplaceLive({self._js_str(id_)}, {self._js_str(html)})")
