...
让 deepseek-desktop 复用 video-agent/core/agnes.py 的 AgnesClient，
根除两份 Agnes 视频客户端逻辑（提交/轮询/下载/限流/三模式/口播注入）。

定位策略：
  - 工作区内：本文件所在目录的 ../video-agent （与 deepseek-desktop 同工作区）
...
def _find_core_parent():
    ...
    候选顺序：
      1. 开发态：本文件所在目录的 ../video-agent （与 deepseek-desktop 同工作区）
...
if os.path.isdir(os.path.join(_VA, "core")) and _VA not in sys.path:
    # 追加（非插入最前）：deepseek-desktop 自身的模块（config 等）仍优先解析，
